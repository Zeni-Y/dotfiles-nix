...
### 7-7. 例外: 「まだ Nix 管理下に無い実ファイル」は上書きされず switch が止まる

7-6 の「強制適用」が効くのは、**すでに Home Manager が管理しているファイル**です。
Nix が初めてそのパスを管理しようとしたときに、そこに**管理外の実ファイル**があると、
Home Manager は黙って消さずに activation ごと停止します。

```
Existing file '/home/zenimoto/.config/gwq/config.toml' would be clobbered
```

これは安全側の挙動ですが、**switch 全体が失敗する**ので初回構築では厄介です。
とくに **自分で設定ファイルを書き出すツール**を、Nix で配る前に一度でも実行していると
確実に踏みます (gwq は `--help` 以外のほぼ全サブコマンドで
`~/.config/gwq/config.toml` を既定値で自動生成します)。

対処は 3 つあり、**使い分けが重要**です。

| 方法 | 挙動 | 向き不向き |
| --- | --- | --- |
| `force = true` (ファイル単位) | 問答無用で上書き。冪等 | **恒久対策**。中身を全部 Nix で宣言していて手元編集に意味が無いファイル向け |
| `switch -b backup` | 既存を `<name>.backup` に退避 | **初回 switch 向け**。`.backup` が既にあると再実行がまた止まるので恒久対策には不向き |
| 手で `rm` してから switch | 一度きり | 中身を確認したうえで捨てたいとき |

このリポジトリでは `home/cli/gwq.nix` が `force = true` を使っています。

```nix
xdg.configFile."gwq/config.toml" = {
  force = true;      # gwq が初回実行時に既定値を書き出すため
  text = '' ... '';
};
```
...
> するなら `force = true` を付けておくと、他のマシンで環境構築するときに
> 「先に触ってしまったせいで switch が止まる」事故を防げます。
