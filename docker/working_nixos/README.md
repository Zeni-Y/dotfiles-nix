...
| Nix                 | イメージビルド時にインストール（installer） | ベースイメージに同梱                          |
| dotfiles 適用       | イメージビルド時に適用済み                  | イメージビルド時に適用済み                    |
| ビルドコンテキスト  | リポジトリルート（flake を焼き込むため）    | リポジトリルート（flake を焼き込むため）      |
| ビルド時間          | 初回は数十分（`BAKE_DOTFILES=0` なら数分）  | 初回は数十分（nixpkgs のビルド／ダウンロード）|
| flake 更新の反映    | `make switch`（または `make build` し直す） | `make switch`（または `make build` し直す）   |
| SSH ポート（既定）  | 2222                                        | 2223（両方同時に起動できるようにずらしてある）|
| パッケージ管理      | Nix 中心（`apt` は逃げ道として使える）      | Nix のみ（`apt` は無い）                      |
| FHS 互換            | あり（プリビルドバイナリがそのまま動く）    | 無し（`/lib64/ld-linux…` などが存在しない）   |
